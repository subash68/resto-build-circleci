...
func (s *orderServiceServer) Finish(ctx context.Context, req *order.FinishedRequest) (*order.Finished, error) {
	if ctx == nil {
	...
	_, err = c.ExecContext(ctx, "UPDATE cart SET cartState = ? where id = ?", FINISHED, req.CartId)
