...
func (s *orderServiceServer) Preparation(ctx context.Context, req *order.InPreparationRequest) (*order.InPreparation, error) {
	if ctx == nil {
		return &order.InPreparation{
			Api: apiVersion,
	...
	_, err = c.ExecContext(ctx, "UPDATE cart SET cartState = ? where id = ?", IN_PREPARATION, req.CartId)
