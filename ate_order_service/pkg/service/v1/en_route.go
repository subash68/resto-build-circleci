...
func (s *orderServiceServer) Route(ctx context.Context, req *order.EnRouteRequest) (*order.EnRoute, error) {
	if ctx == nil {
	...
	_, err = c.ExecContext(ctx, "UPDATE cart SET cartState = ? where id = ?", EN_ROUTE, req.CartId)
