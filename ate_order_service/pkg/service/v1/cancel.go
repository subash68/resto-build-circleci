...
func (s *orderServiceServer) Cancel(ctx context.Context, req *order.CanceledRequest) (*order.Canceled, error) {
	if ctx == nil {
	...
	_, err = c.ExecContext(ctx, "UPDATE cart SET cartState = ? where id = ?", CANCELED, req.CartId)
